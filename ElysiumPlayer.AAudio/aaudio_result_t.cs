 ...
 */
    ...
    public class AAudioException: Exception
    {
        public aaudio_result_t Error { get; }
        public AAudioException(aaudio_result_t error): base($"AAudio error: {error}")
        {
            Error = error;
        }
    }
    public static class aaudio_result_t_extensions
    {
        public static void Check(this aaudio_result_t status)
        {
            if (status != aaudio_result_t.AAUDIO_OK)
            {
                throw new AAudioException(status);
            }
        }
    }
